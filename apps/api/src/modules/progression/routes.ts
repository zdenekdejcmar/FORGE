...
const progressionRoutes: FastifyPluginAsync = async (app) => {
  app.get('/character', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const row = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.userId, userId));
  ...
  app.get('/arenas', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const result = await Promise.all(arenaRows.map(async (arena) => {
      const row = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, arena.id));
  ...
  app.get('/arenas/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const row = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, arena[0].id));
    const progress = getXpProgressForLevel(Number(row[0]?.totalXp ?? 0));
    reply.send({
      ...arena[0],
      level: progress.level,
