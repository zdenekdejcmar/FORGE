...
const arenaRoutes: FastifyPluginAsync = async (app) => {
  app.post('/', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (!parsed.success) throw new ValidationError('Invalid arena payload.');
  ...
  app.get('/', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const result = await Promise.all(rows.map(async (arena) => {
      const totalXpRow = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, arena.id));
      const activeQuestCount = await app.db.select({ count: sql<number>`COUNT(*)::int` }).from(quests).where(and(eq(quests.arenaId, arena.id), eq(quests.status, 'ACTIVE')));
      const completedQuestCount = await app.db.select({ count: sql<number>`COUNT(*)::int` }).from(quests).where(and(eq(quests.arenaId, arena.id), eq(quests.status, 'COMPLETED')));
      return {
        ...arena,
  ...
  app.get('/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (item.length === 0) throw new NotFoundError('ARENA_NOT_FOUND', 'Arena not found.');

    const totalXpRow = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, item[0].id));
  ...
  app.patch('/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (!parsed.success) throw new ValidationError('Invalid arena update payload.');
  ...
  app.delete('/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const questCount = await app.db.select({ count: sql<number>`COUNT(*)::int` }).from(quests).where(eq(quests.arenaId, item[0].id));
    const xpCount = await app.db.select({ count: sql<number>`COUNT(*)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, item[0].id));
