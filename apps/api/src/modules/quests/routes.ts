...
import { arenas, characters, questCompletions, quests, users, xpTransactions } from '../../../../../packages/db/src/schema';
import { questSchema } from '../../../../../packages/validation/src/index';
import { getXpProgressForLevel } from '../../../../../packages/domain/src/index';
import { ConflictError, ForbiddenError, NotFoundError, ValidationError } from '../../shared/errors/http.js';

const questRoutes: FastifyPluginAsync = async (app) => {
  app.post('/', { preHandler: [app.authenticate] }, async (request, reply) => {
    const parsed = questSchema.safeParse(request.body);
    if (!parsed.success) throw new ValidationError('Invalid quest payload.');

    const arena = await app.db.select().from(arenas).where(eq(arenas.id, parsed.data.arenaId)).limit(1);
    if (arena.length === 0) throw new NotFoundError('ARENA_NOT_FOUND', 'Arena not found.');
  ...
  app.get('/', { preHandler: [app.authenticate] }, async (request, reply) => {
    const filters: any = { userId: request.user!.id };
  ...
  app.get('/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (row.length === 0) throw new NotFoundError('QUEST_NOT_FOUND', 'Quest not found.');
    reply.send(row[0]);
  });
  ...
  app.patch('/:id', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    if (existing.length === 0) throw new NotFoundError('QUEST_NOT_FOUND', 'Quest not found.');

    const parsed = questSchema.partial().safeParse(request.body);
    if (!parsed.success) throw new ValidationError('Invalid quest update payload.');

    const updateValues: any = { ...parsed.data, updatedAt: new Date() };
    if (parsed.data.dueAt !== undefined) updateValues.dueAt = parsed.data.dueAt ? new Date(parsed.data.dueAt) : null;
  ...
  app.post('/:id/complete', { preHandler: [app.authenticate] }, async (request, reply) => {
    try {
      ...
      if (questRow.length === 0) throw new NotFoundError('QUEST_NOT_FOUND', 'Quest not found.');
      ...
      if (character.length === 0) throw new NotFoundError('CHARACTER_NOT_FOUND', 'Character not found.');
      ...
      const arenaTotal = await app.db.select({ totalXp: sql<number>`COALESCE(SUM(amount), 0)::int` }).from(xpTransactions).where(eq(xpTransactions.arenaId, quest.arenaId));
      ...
      // DEBUG: log completion response payload for e2e troubleshooting
      app.log.info({ event: 'quest-complete-response', payload });
      ...
      app.log.error({ event: 'quest-complete-error', questId, userId, error: err instanceof Error ? err.stack : err });
      throw err;
    }
  });
  ...
  app.post('/:id/abandon', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const existing = await app.db.select().from(quests).where(and(eq(quests.id, params.id), eq(quests.userId, request.user!.id))).limit(1);
    if (existing.length === 0) throw new NotFoundError('QUEST_NOT_FOUND', 'Quest not found.');
    if (existing[0].status === 'COMPLETED') throw new ConflictError('QUEST_ALREADY_COMPLETED', 'Completed quests cannot be abandoned.');
